...
@implementation DIYImageScrollView
{
    int _iCount;
    NSTimer *_timer;
    ...
    BOOL _bMarrImageListChanged;//图片数组是否改变
}
...
- (void)showPhotoViewAtIndex:(int)index
{
    ...
    [photoView sd_setImageWithURL:[NSURL URLWithString:strPicture] placeholderImage:strPicture.length?[UIImage imageNamed:strPicture]:nil];
...
-(void)ImageScrollUserClicked:(UITapGestureRecognizer  *)sender
{
    ...
    {
         [_delegate ImageScroll:self UserClickedAtPage:indexPic];
    }
    if ([_delegate respondsToSelector:@selector(ImageScrollUserClickedAtPage:)])
    {
        [_delegate ImageScrollUserClickedAtPage:indexPic];
    }
}
